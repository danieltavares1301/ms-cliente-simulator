...
| O10 | Rajada concorrente Cliente/PAC | Script dedicado (`scripts/stress-o10-concurrent-cliente-update.ts`, `npm run stress:o10`) | Implementado como stress técnico fora do catálogo declarativo e validado ao vivo contra `mrv-devDan`: 12/12 `cliente-update` retornaram HTTP 200 em múltiplas rodadas, sem `UNABLE_TO_LOCK_ROW`/`DmlException` surfaced; o vencedor final variou entre `Cliente Concorrente 10/11/12`, logo a corrida é real no estado final. Evidência: `docs/phase-6/o10-stress-concorrencia.md`. |

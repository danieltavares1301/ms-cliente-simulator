...
function createRequest(body?: string, headers?: HeadersInit): Request {
  return new Request(
    'https://simulator.example.com/api/ms-clientes/contestacao-documentos',
    {
      method: 'POST',
      headers: {
...
describe('createContestacaoDocumentosCallbackHandler', () => {
  it.each([
    {
    ...
    {
      name: 'the contestacao documentos callback flag is disabled',
      environment: {
        ...validEnvironment,
    ...
    const handler = createContestacaoDocumentosCallbackHandler({
      environment,
    ...
    await expect(response.json()).resolves.toStrictEqual({
      error: {
        code: 'CONTESTACAO_DOCUMENTOS_CALLBACK_DISABLED',
        message: 'Contestacao documentos callback is disabled',
  ...
  it('returns 422 when the JSON payload does not satisfy the contestacao documentos contract', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    const handler = createContestacaoDocumentosCallbackHandler({
      environment: validEnvironment,
  ...
  it('returns 201 and emits a structured log without leaking the bearer token', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    const handler = createContestacaoDocumentosCallbackHandler({
      environment: validEnvironment,

...
`data.IdCliente` é o identificador de negócio obrigatório. Evidência real observada em 20/09/2026 na org `mrv-devDan`: a exception `NotificacaoClienteException:[]: O Atributo obrigatório "IdCliente" não foi informado!` foi registrada em `NotificacaoCliente.atributosObrigatoriosForamInformados` (linha 283) mesmo com `data.idcliente` preenchido, porque `EventGrid.iniciar()` repassa `data.id` para `NotificacaoCliente.getSObjectExistente(...)` e o Apex só faz o fallback `this.idExterno = data.idcliente` quando `data.id` está em branco. Um log real de sucesso da mesma org confirmou o comportamento: quando o evento omite `data.id`, `data.idcliente` é usado corretamente. Portanto, fixtures e contratos do simulador **devem omitir `data.id`** para `cliente-*`, `contato-*` e `endereco-*`; não basta manter consistência entre os dois campos. `data.IdProspectSalesforce` e `data.NumeroCPF` participam da resolução de Account/Lead em todos os tipos, mas não têm validação explícita de presença.
...
|---|---|---|
| `cliente-insert` | `id`, `IdCliente`, `IdProspectSalesforce`, `NumeroCPF`, `DataAlteracao`, `Categoria`, `CodSAP`, `DataNascimento`, `Escolaridade`, `EstadoCivil`, `Naturalidade`, `NomeCompleto`, `NomeMae`, `NomePai`, `NumeroPIS`, `NumeroDocumento`, `OrgaoEmissorDocumento`, `Profissao`, `SistemaOrigem`, `TipoPessoa`, `TituloEleitor`, `Sexo` | Apenas `IdCliente` é explicitamente obrigatório. Os demais são condicionais. Para criar Person Account válida no cenário normal, a fixture deve fornecer os campos exigidos pela configuração Salesforce, especialmente o nome; isso será fechado pela matriz FLS/metadata antes do E2E. **No simulador, `data.id` deve ser omitido mesmo aparecendo na leitura do Apex, porque seu preenchimento impede o fallback para `IdCliente`.** |
| `cliente-update` | Mesmos campos de `cliente-insert` | Mesma validação e mesmo mapeamento. Não há semântica distinta de patch além do upsert/update encontrado. `data.id` continua proibido nas fixtures pelo motivo acima. |
| `contato-insert` | `id`, `IdCliente`, `IdProspectSalesforce`, `NumeroCPF`, `DataAlteracao`, `TipoContato`, `Descricao` | `IdCliente` é explicitamente obrigatório. `TipoContato` + `Descricao` são necessários para efeito. Valores tratados: `Email`, `Celular`, `Telefone`. **`data.id` também deve ser omitido.** |
| `contato-update` | Mesmos campos de `contato-insert` | Mesmo ramo e mesmos valores de `TipoContato`. `data.id` continua proibido nas fixtures. |
| `endereco-insert` | `id`, `IdCliente`, `IdProspectSalesforce`, `NumeroCPF`, `DataAlteracao`, `TipoEndereco`, `IdCidade`, `Logradouro`, `NumeroCEP`, `Bairro`, `Numero` | `IdCliente` é explicitamente obrigatório. Só há efeito de endereço quando `TipoEndereco` é exatamente `COBRANCA`; os demais campos são condicionais. `IdCidade` resolve `Cidade__c` e deriva cidade/UF; país vira `Brasil` se ausente. **`data.id` também deve ser omitido.** |
| `endereco-update` | Mesmos campos de `endereco-insert` | Mesmo ramo e mesma comparação temporal. `data.id` continua proibido nas fixtures. |
